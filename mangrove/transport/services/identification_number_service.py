...
class IdentificationNumberService(object):
    def __init__(self, dbm):
        self.dbm = dbm
    def save_identification_number(self, form_code, values, location_tree):
        form_model = get_form_model_by_code(self.dbm, form_code)
        return self._submit_subject(form_model, values, location_tree)
    ...
    def _submit_subject(self, form_model, values, location_tree):
        try:
            ...
            response = handler.handle(form_model, cleaned_data, errors,  [], location_tree)
            return response
