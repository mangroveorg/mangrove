...
from mangrove.contrib.registration_validators import MobileNumberValidationsForReporter, AtLeastOneLocationFieldMustBeAnsweredValidator, EmailFieldValidatorForReporter, MandatoryValidatorForReporter
from mangrove.form_model.validators import MandatoryValidator
from mangrove.datastore.datadict import get_or_create_data_dict
from mangrove.form_model.field import HierarchyField, TextField, TelephoneNumberField, GeoCodeField, email_regex
...
def construct_global_registration_form(manager):
    ...
    question1 = HierarchyField(name=ENTITY_TYPE_FIELD_NAME, code=ENTITY_TYPE_FIELD_CODE,
                               label="What is associated subject type?", ddtype=entity_id_type,
    ...
    question7 = TextField(name=EMAIL_FIELD, code=EMAIL_FIELD, label="What is the subject's email",
                          ...
                          constraints=[TextLengthConstraint(max=50)], required=False)
    form_model = FormModel(manager, name=GLOBAL_REGISTRATION_FORM_CODE, form_code=REGISTRATION_FORM_CODE, fields=[
        question1, question2, question3, question4, question5, question6, question7], is_registration_model=True,
                           entity_type=["registration"],
                           validators=[MandatoryValidatorForReporter(), MobileNumberValidationsForReporter(),
                                       EmailFieldValidatorForReporter(),
