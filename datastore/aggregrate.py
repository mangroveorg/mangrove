...
# currently it assumes one to one mapping between form code and entity type and hence only filter on form code
    ...
    values = []
    for row in rows:
        if _should_include_row(row, group_level, form_code, start_time, end_time):
...
def _end_time_filter(row, end_time):
    if end_time is None:
        return True
    epoch = int(time.mktime(time.strptime(end_time, '%d-%m-%Y %H:%M:%S')))*1000
    return row.key[BY_VALUES_EVENT_TIME_INDEX] <= epoch


def _should_include_row(row, group_level, form_code, start_time, end_time):
    return _form_code_filter(row, group_level, form_code) and _start_time_filter(row, start_time) and \
