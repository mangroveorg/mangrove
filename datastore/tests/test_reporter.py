...
class TestReporter(TestCase):
    ...
    def test_should_load_reporter_given_tel_number(self):
        saved_r2 = find_reporter(self.manager,"8888567890")
        self.assertIsNotNone(saved_r2)
        self.assertEquals(saved_r2["first_name"],"B")
        self.assertEquals(saved_r2["telephone_number"],"8888567890")
    ...
    def test_should_raise_exception_if_multiple_reporters_for_a_number(self):
        with self.assertRaises(MultipleReportersForANumberException):
            reporter = find_reporter(self.manager,"1234567890")
